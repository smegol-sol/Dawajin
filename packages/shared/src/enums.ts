...
/**
 * **حالة الدفعة — ثلاثٌ بعد فصل الإنشاء عن البدء** (القرار 160 «عاشرًا» ٣).
 *
 * **و«قيد الوصول» ليست حالةً تجميلية بل موضعُ حكمٍ كامل:** الحكم يفصل فعلين
 * كانا واحدًا — **المشرف أو المالك يُنشئ سجلّ الدفعة عند التوزيع**، **وتأكيدُ
 * المربّي هو ما يبدأها**: «الدفعة تبدأ حين تصل الطيور فعلًا». **فبين الفعلين
 * صفٌّ قائم لدفعةٍ لم تبدأ** — ولم تكن له قيمةٌ تمثّله، **فكان الإنشاء يُنتج
 * «نشطة» كاذبة**.
 *
 * **وأثرُه على العدّاد والمقام معًا:** دفعةٌ «قيد الوصول» **بلا مستلمٍ مؤكَّد
 * وبلا تاريخ بدء** — ويفرض ذلك `batches_arrival_shape_ck` على القاعدة لا في
 * الخدمة.
 *
 * **والترتيب مقصود — «قيد الوصول» أولًا:** هو حال الصفّ يوم يُخلق، والقيمُ
 * تمضي إلى الأمام ولا تعود.
 */
export const BATCH_STATUS = ["قيد الوصول", "نشطة", "منتهية"] as const;
export type BatchStatus = (typeof BATCH_STATUS)[number];

/**
 * **حالات الدفعة التي فيها طيرٌ في العنبر — قائمةٌ موجبة** (القرار 160
 * «عاشرًا» ٥: «التنبيهات تتبع الوجود الفعلي للطيور لا حالة السجل»).
 *
 * **وعنبرٌ وصلته طيورٌ ولم تُؤكَّد ليس فارغًا** — وقراءةُ «نشطة» وحدها تجعله
 * كذلك، **فيسقط من كل تنبيهات النظام لأن أحدًا لم يضغط زرًّا**، **ويُقبل
 * إرجاعُه إلى «جاهز للإسكان» وفيه طير**.
 *
 * **وقائمةٌ موجبة لا شرطٌ سالب** (`status <> 'منتهية'`) **لأن الرابعة تُضاف
 * غدًا فتدخل بالسكوت** — نمط `ASSIGNMENT_SCOPED_ROLES` والقرار 184.
 */
export const BATCH_STATUSES_WITH_BIRDS: readonly BatchStatus[] = ["قيد الوصول", "نشطة"];
...
/**
 * فئات الأصناف — **سبعٌ بعد قسمة «مستلزمات»** (القرار 231، والتنفيذ 260).
 *
 * **وفُصلت «معقمات ومطهرات» أولًا** (#161 «ثالث عشر» ٧)، **ثم قُسمت
 * «مستلزمات» إلى تشغيلٍ وإنشائيّ** — **وهي الحركة نفسها للعلّة نفسها**:
 * **كتلةٌ تضمّ ما يُسمح به وما يُمنع لا تقبل حكمًا**، **فالقسمة شرطُ إمكان
 * القاعدة لا ترفٌ في التسمية**.
 *
 * **والقسمة أوجبها تناقضٌ بين قرارين قائمين** (231 §١): النصُّ ينفي المعدّات
 * عن مخزن العنبر، **والقرارُ يوجب أن تُجمَّع فيه الأكياس الفارغة وفئتُها
 * «مستلزمات»** — **فمنعُ الكتلة يمنع الكيس من موضعه، والسماحُ بها يُدخل ما
 * نُفي**.
 *
 * **والمسموح في مخزن العنبر ستٌّ، والممنوع «معدات ومستلزمات إنشائية» وحدها**
 * — و`HOUSE_WAREHOUSE_CATEGORIES` أدناه هي القائمة المفروضة.
 *
 * **وصمتٌ معلن:** **لا `z.enum` يربط هذه القائمة في `apps/api/src`**، **فلا
 * يحرسها فاحصُ «صحة enum»** — **وأولُ مخطط تحقّقٍ يربطها يُفعِّل حراستَه
 * كاملة** (القرار 231).
 */
export const PRODUCT_CATEGORY = [
  ...
  "معقمات ومطهرات",
  "مستلزمات تشغيل",
  "معدات ومستلزمات إنشائية",
] as const;
export type ProductCategory = (typeof PRODUCT_CATEGORY)[number];

/**
 * **ما يحمله مخزن العنبر — قائمةٌ موجبة** (القرار 231، والفرض 260).
 *
 * **والممنوع واحدةٌ لا أكثر: «معدات ومستلزمات إنشائية»** — **ولا يُمنح غيرُها
 * بالسكوت**: فئةٌ ثامنة تُضاف غدًا **تبقى خارج القائمة حتى تُدرَج بقرار**
 * (نمط `ASSIGNMENT_SCOPED_ROLES`، والقرار 184).
 */
export const HOUSE_WAREHOUSE_CATEGORIES: readonly ProductCategory[] = [
  "علف",
  "دواء",
  "لقاح",
  "فيتامين",
...
export const INVENTORY_MOVEMENT_TYPE = [
  ...
  "صرف خارجي",
  /**
   * **الرابع عشر — الطرف الموجب لتفريغ الكيس** (القرار 212، على #161 «عاشرًا»:
   * «**استهلاك كيس ينقل واحدًا من الأول إلى الثاني**»).
   *
   * **والاستهلاك طرفان لا طرف:** `استهلاك يومي` **(−) على صنف العلف**، و
   * **`تفريغ كيس` (+) على صنف الكيس الفارغ** — **وهما على صنفين مختلفين**،
   * فلا يوجد نوعٌ قائم يصلح للطرف الموجب.
   */
  "تفريغ كيس",
] as const;
 ...
 */
/**
 * **حالة الكيس الفارغ** (القرار 212، على #161 «عاشرًا»): **صالح · تالف لا غير**.
 *
 * **وبلا حالة التالف يُحسب التلف مفقودًا ويُتَّهم المربّي بلا ذنب** — **ولذلك
 * وُجدت**. **والتالف يبقى في رصيد الفارغ بوصف مختلف ولا يخرج منه** («ثالث عشر»
 * ٨): **تسجيله هالكًا يُنقص الرصيد فتختلّ معادلة التحقق فتفقد قدرتها على كشف ما
 * بُنيت له** — **ويمنعه حارسٌ في القاعدة لا تعليقٌ في المخطط** (درس القرار 203).
 */
export const EMPTY_BAG_CONDITION = ["صالح", "تالف"] as const;
export type EmptyBagCondition = (typeof EMPTY_BAG_CONDITION)[number];
