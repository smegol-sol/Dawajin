...
/**
 * **الحالات الثلاث المسموح ميلادُ العنبر فيها** — قرار المالك (القرار 220 شقّ ٣)،
 * تنفيذًا للقرار 186: **تُختار الحالة الابتدائية صراحةً ولا تُفترض**.
 *
 * **قائمة موجبة لا شرطًا سالبًا** (نمط 184 و#161): حالةٌ تُضاف إلى
 * `HOUSE_STATUS` غدًا **لا تصير مولودةً بالسكوت** — تُدرَج هنا بعلّتها أو
 * تبقى خارج الميلاد.
 */
export const HOUSE_CREATABLE_STATUSES = [
  "جاهز للإسكان",
  "تحت الصيانة",
  "معطّل",
] as const satisfies readonly HouseStatus[];

export type HouseCreatableStatus = (typeof HOUSE_CREATABLE_STATUSES)[number];

export function isHouseCreatableStatus(status: HouseStatus): status is HouseCreatableStatus {
  return (HOUSE_CREATABLE_STATUSES as readonly HouseStatus[]).includes(status);
}

/**
 * **علّةُ منع كلِّ حالةٍ من الميلاد — مكتوبةٌ مع القاعدة لا في قرارٍ بعيد عنها**
 * (نمط `source` في جدول الانتقالات).
 *
 * **والعلّتان صنفان لا صنفٌ واحد:** حالتان **تفترضان دفعة**، وحالتان
 * **تفترضان دورة تجهيز** — **وكلتاهما تاريخٌ لا يملكه عنبرٌ وُلد للتوّ**.
 * **والأثر مقيسٌ على الجدول لا مقدَّرًا:** لا واحدة من الأربع تبلغ «جاهز
 * للإسكان» بمسارٍ أمامي، **لأن كل مسارٍ أمامي يمرّ بدفعةٍ أو دورةٍ لا وجود
 * لها** — فيولد العنبر عالقًا.
 */
export const HOUSE_BIRTH_EXCLUSIONS: Readonly<Record<string, string>> = {
  مشغول:
    "يفترض دفعةً نشطة، ولا دفعة في عنبرٍ وُلد للتوّ. **ومخرجه الأمامي «تحت الإخلاء» أثرُ تصفية الدفعة** — والتصفية تُغلق دفعةً غير موجودة، فلا تقع أبدًا",
  "تحت الإخلاء":
    "يفترض دفعةً أُغلقت للتوّ ودورةَ تجهيزٍ أُنشئت معها (§14.6)، **ولا واحدة منهما موجودة**. **ومخرجه الأمامي يقع في «تحت التنظيف والتطهير» فيرث عطبها**",
  "تحت التنظيف والتطهير":
    "**لا دورة فلا خطواتٌ تُكمَل فلا انتقالٌ يقع** (القرار 221): مخرجه الأمامي الوحيد `prep-complete` **تلقائيٌّ لا يُطلب يدويًّا**، ومُطلِقه إكمالُ خطوةٍ من دورةٍ لا وجود لها",
  "في فترة الراحة":
    "**الراحة ذيلُ دورة تجهيز لا نقطةَ بداية** (قرار المالك، القرار 220). ومخرجه الأمامي `rest-confirm` يحرسه `no_open_prep_cycle`: **لا مدةَ انقضت لأنه لا مدةَ قرّرها أحد**",
};
