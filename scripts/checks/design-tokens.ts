 ...
 *
 * **فرُفع الارتفاع عن هذه الثلاث وحدها، وبقي في الباقي كلِّه** (٦٣ كتلة).
 *
 * ## وهو استثناءٌ مسمًّى لا إصلاحُ سببٍ جذريّ — **الحدُّ يُعلن**
 *
 * **العطبُ اقتطاعٌ أفقيّ لا قصٌّ رأسيّ** — الحروفُ كاملةُ الارتفاع والنصُّ
 * يُبتر من آخره. **وثلاثةُ علاجاتٍ جُرّبت وسقط اثنان:**
 * `includeFontPadding: false` و`Math.ceil` **كلاهما رُئي سليمًا مرّةً ثم
 * اقتُطع** — **إنذارٌ كاذبٌ من نظرةٍ واحدة**، وهو **رابعُ وقوعٍ لعطبٍ في أداة
 * القياس لا في المفحوص** (بعد ثلاثةٍ في 289)، **والأداةُ هنا النظرةُ الواحدة**.
 *
 * **ورفعُ الارتفاع وحدَه صمد لثلاث ملاحظاتٍ في حالاتٍ مختلفة** — بروتوكولُ
 * القرار 296: **بدءٌ بارد ← انتقالٌ وعودة ← إغلاقٌ وفتح**.
 *
 * **والسببُ الجذريّ غير معروف** — والفرضية القائمة **[غير مقيسة]**: ارتفاعُ
 * السطر يُفسد قياسَ العرض في **نصٍّ سطرُه واحدٌ في حاويةٍ بلا فسحة**؛
 * **و«دواجن» سطرٌ واحد بارتفاعٍ وهي سليمة، ففسحتُها واسعة**.
 *
 * **وشرطُ سقوط الاستثناء: أداةُ تؤكّد على شجرة Yoga الأصلية** (§7-ب البند 37)
 * — **فحينها يُقاس السببُ لا العرَض، ويُبنى العلاجُ الدائم ويُحذف هذا**.
 *
 * **ولا يُوسَّع بالسكوت:** كتلةٌ رابعة تُضاف هنا **تلزمها ملاحظاتٌ ثلاث
 * بنفس البروتوكول**، لا نظرةٌ واحدة.
 */
const LINE_HEIGHT_EXEMPT = new Set([
...
function styleBlockViolations(relPath: string, content: string): string[] {
  ...
  for (const match of content.matchAll(STYLE_BLOCK)) {
    ...
    if (!body.includes("lineHeight") && !LINE_HEIGHT_EXEMPT.has(`${relPath}:${name}`)) {
      found.push(
