...
describe("مصفوفة الصلاحيات — إدارة المستخدمين للمالك وحده في هذه الدفعة", () => {
  ...
  /**
   * **وأمين المخزن يُرفض قبل أن يبلغ حارس الدور** — من الفرض المركزي: هو خارج
   * `FULL_VISIBILITY_ROLES` وخارج `ASSIGNMENT_SCOPED_ROLES` معًا، **فكل طلب
   * `/api/*` منه 403 أيًّا كان مساره** (القرار 194، والحدّ المعلن في 227).
   *
   * **فهذا الصفّ لا يقيس `requireRole` — وإسقاطُ الحارس لا يُسقطه، مقيسًا لا
   * مفترَضًا.** ويبقى لأنه يقيس شيئًا آخر صحيحًا: **أن الباب مغلق فعلًا**.
   */
  /**
  ...
  it("دور storekeeper ← 403 **من الفرض المركزي لا من حارس الدور**", async () => {
    ...
    expect((list.body as ErrorBody).message).toBe(CENTRAL_GUARD_MESSAGE);
  });
});
