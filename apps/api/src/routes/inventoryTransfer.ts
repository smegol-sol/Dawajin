...
export function inventoryTransferRouter(db: Database): Router {
  ...
  //
  // **وأمين المخزن أُضيف بالقرار 259 — «تنفيذًا لا أمرًا»** (§12.2 صفّ
  // «تحويل»). **وخانةٌ كانت مكتوبةً ولا تُبلَغ**: كان محجوبًا حتى بُني تأكيدُ
  // المحطتين (258)، **لأن فتحَ التنفيذ قبل المؤكِّد يُخرج بلا تأكيد**.
  //
  // **ولا يوسّع نطاقَه حرفًا:** الفرضُ المركزي يحلّ التحويلَ إلى **مخزنه
  // المرسِل** (القرار 258)، **وإسنادُه مخزنٌ بعينه** (254) — **فلا ينفّذ إلا
  // من مركزيّه**. **والأمرُ يبقى ممنوعًا عليه** (`requireRole` على مسار
  // الإصدار، وحارسُ «أمين حفظ لا آمر صرف» خلفه).
  router.post(
