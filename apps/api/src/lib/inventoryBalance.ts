 ...
 * بعدها.
 *
 * **وحدٌّ معلن يُكتب ولا يُدَّعى غيره — وقد تحرّك تحته البناءُ فصُحِّح
 * (القرار 267):** **كان هنا «لا مسار مخزون يكتب حركةً اليوم»**، **وبطل**:
 * **ثلاثُ خدماتٍ تكتب في `inventory_movements` اليوم** — الاستلام والتحويل
 * وتأكيدُ الاستلام. **والقفل يبقى شرطًا مكتوبًا لا مفروضًا بحارسٍ آليّ**،
 * **فيُقرأ التزامًا واجبًا على كل كاتبٍ قائم لا وعدًا على كاتبٍ قادم.**
 * **و`writeBalanceSnapshot` نفسها بلا مستدعٍ إنتاجيّ بعد، فأثرُ الشرط لم يقع
 * بعد** — **يسقط هذا الحدّ يوم يستدعيها أول مسار** (القرار 223).
 * @limit no-caller writeBalanceSnapshot
 *
