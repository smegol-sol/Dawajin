 ...
 * (القرار 229).
 *
 * **وحدُّه سقط بالقرار 276 كما أُعلن:** `GET /api/chick-shipments/:shipmentId`
 * بُني، **وقراءتُه مفلترةٌ بالإسناد في `chickShipmentReadService`** — المالكُ
 * كلَّ التوزيعات، والمشرفُ توزيعاتِ مزارعه، والمربّي عنابرَه؛ **وشحنةٌ لا
 * يبلغ منها شيئًا تُردّ 403 لا قائمةً فارغة** (#129). **فالوجودُ هنا،
 * والرؤيةُ هناك — طبقتان لا واحدة.**
 *
