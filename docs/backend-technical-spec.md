...
**وثلاثة مستويات لا مستويان (القرار #128، ثم القرار 198):** المربّي يُسند **بالعنبر** (`house_id`)، والمشرف والطبيب **بالمزرعة** (`farm_id`)، **والمشرف وحده بمخزن الموقع** (`warehouse_id` — القرار 247). صفٌّ واحد يحمل مستوى واحدًا حتمًا: `CHECK` يجمع الثلاثة ويشترط `= 1`.
